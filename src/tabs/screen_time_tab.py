...
class ScreenTimeTab(BaseNudgyTab):
    ...
    def set_history_sec(self, history_sec: int=0) -> None:
        ...
        for a in self._apps:
            self._total_time_sec += a.num_timestamps(self.get_history_sec()) * self.REFRESH_RATE_SEC
    ...
    def update_time_percent(self, enable_sorting_after: bool=True, rows: list[int] | None=None) -> None:
        ...
        for r in rows:
            ...
            time_percent = 0
            try:
                time_percent = (app.num_timestamps(self.get_history_sec()) * self.REFRESH_RATE_SEC * 100) / self._total_time_sec
